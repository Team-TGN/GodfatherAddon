...
import TheGodfather.database.afkdb as db
from TheGodfather.helpers.pyrohelper import user_afk
from TheGodfather.plugins.alive import get_readable_time
from TheGodfather.helpers.utils import get_message_type, Types
...
async def afk(app: Client, message):
    ...
    if not arg:
        ...
        reason = arg
    await db.set_afk(True, afk_time, reason)
...
async def afk_mentioned(app: Client, message):
    global MENTIONED
    afk_time, reason = await db.afk_stuff()
