...
from TheGodfather.helpers.pyrohelper import get_arg, welcome_chat
import TheGodfather.database.welcomedb as db
...
@Client.on_message(filters.command("clearwelcome", PREFIX) & filters.me)
async def welcome(app: Client, message):
    await db.clear_welcome(str(message.chat.id))
...
async def new_welcome(app: Client, message):
    msg_id = await db.get_welcome(str(message.chat.id))
    caption = ""
    men = ""
...
@Client.on_message(filters.command("setwelcome", PREFIX) & filters.me)
async def setwelcome(app: Client, message):
    ...
    await db.save_welcome(str(message.chat.id), msg_id)
