...
from TheGodfather.database.afkdb import get_afk_status
from TheGodfather.database.pmpermitdb import get_approved_users, pm_guard
import TheGodfather.database.welcomedb as fucking
import shlex
...
async def welcome_chat(filter, client: Client, message: Message):
    to_welcome = await fucking.get_welcome(str(message.chat.id))
    if to_welcome:
        return True
    else:
        return False
